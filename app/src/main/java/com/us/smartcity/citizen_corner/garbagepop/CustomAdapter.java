...
public class CustomAdapter extends BaseAdapter {
    ...
    public View getView(int pos, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            grid = new View(c);
            grid = inflater.inflate(R.layout.gridgarbage, null);

        } else {
            grid = (View) convertView;
        }

        ImageView imageView = (ImageView)grid.findViewById(R.id.gIcon);

        imageView.setImageResource(img[pos]);
        TextView textView = (TextView) grid.findViewById(R.id.gTitle);
